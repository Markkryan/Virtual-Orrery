...
void RenderingEngine::RenderScene(const std::vector<Geometry>& objects) {
	...
	for (const Geometry& g : objects) {
		...
		glUniform1i(g.uniformlocation, 0);

		GLuint kduniform = glGetUniformLocation(shaderProgram, "kd");
		GLuint ksuniform = glGetUniformLocation(shaderProgram, "ks");

		glUniform1f(kduniform, g.kd);
		glUniform1f(ksuniform, g.ks);

		glm::mat4 m = g.model * g.planet_reference *  g.orbit * g.reference * g.tilt * g.axis;
